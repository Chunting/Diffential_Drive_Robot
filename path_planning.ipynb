...
### STATE CLASS
class state:
  def __init__(self,x,y,theta,g,f,h,vr,vl,vel):
    self.x=x
    self.y=y
    self.theta=theta
    self.g=g
...
def new_state_transition(current_state,goal,speed):
    next_states = []
    delta_angles = velocity_augmentation(current_state.vel,offset=0.2)
    DT=1.0/speed
    for delta in delta_angles:
        ...
        f2     = g2 + h2
        new_state=state(x2,y2,theta2,g2,f2,h2,delta[0],delta[1],speed)
      ...
      if( is_within_grid(nxt_node[0],nxt_node[1])):
        if(grid[nxt_node[0]][nxt_node[1]]==0 and expand_grid[nxt_node[0]][nxt_node[1]]!='*'):
          if(A_vehicle_is_safe(vehicle_point_A,np.array([nxt_node]),grid)):
            g2=old_state.g+1 + ad
            new_state=state(nxt_node[0],nxt_node[1],0,g2,0,0,5.0,5.0,5.0)
...
def show_animation(path, start, goal,vehicle_pt_obj_act):
  ...
  vr +=[p.vr for p in path]
  vl +=[p.vl for p in path]
  ...
  for ii in range(0,len(x),5):
      ...
      #plt.savefig()
      #save +=1
      plot_car(x[ii], y[ii], yaw[ii],vr[ii])
      plt.grid(True)
      plt.axis("equal")
      plt.pause(0.01)
...
def search(start,goal_node,present_heading,grid,speed):
  ...
  current_state = state(vehicle_pt_obj.center[0], vehicle_pt_obj.center[1], present_heading, 0.0, 0.0, 0.0,5.0,5.0,5.0)
  if(build_cost_map(state(goal_node.x,goal_node.y,0,0,0,0,5.0,5.0,5.0),goal(start[0],start[1]),grid)):
...
## CALL SEARCH
search([5,5],goal(25,60),np.pi/2,GRID_TEST,60)
print('1')
